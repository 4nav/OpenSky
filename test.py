...
from quant import calc_enchant_cost, calc_gemstone_cost, get_enchant_cost_cap, get_daily_volume, volume_tier, LIQ_THRESHOLD
...
def load_bazaar_prices(conn):
    ...
    return {
        pid: {"instabuy": instabuy, "instasell": instasell, "buy_volume": bv, "sell_volume": sv}
...
def fetch_sample_rows(conn, item_id, sample_size):
    if item_id:
        ...
        # bias the sample toward rows that actually have modifiers worth inspecting
        query = """
            SELECT item_id, price, quantity, enchantments, hot_potato_count, rarity_upgrades,
                   gemstones, reforge, dungeon_item_level
            FROM ended_auctions
...
def raw_enchant_book_value(bazaar_prices, enchants_json):
    """Sum of raw enchant book bazaar cost with NO liquidity discount applied - lets us see
    what the enchant cost WOULD be before the liquidity multiplier zeroes/shrinks it."""
    from quant import get_enchant_true_cost
    if not enchants_json:
        return 0
    try:
        raw = json.loads(enchants_json)
    except (json.JSONDecodeError, TypeError):
        return 0
    enchants = {e["type"]: e["level"] for e in raw} if isinstance(raw, list) else raw
    return sum(get_enchant_true_cost(bazaar_prices, name, level) for name, level in enchants.items())
...
def inspect_row(conn, row, bazaar_prices, gemstone_costs, reforge_stones, reforge_name_lookup, item_rarities, essence_costs):
    ...
    raw_enchant_value = raw_enchant_book_value(bazaar_prices, enchants_json)
    ...
    print(f"  daily_vol: {daily_vol:.2f} ({volume_tier(daily_vol)}) | liq_score: {liq_score:.3f} (raw enchant value {raw_enchant_value:,.0f} x liq_score = enchant_cost) | enchant_cap: {cap}")
    ...
    if base_price <= 0:
        print(f"  !! WARNING: base_price <= 0")
    if daily_vol == 0 and raw_enchant_value > 0:
        print(f"  !! NOTE: daily_vol is 0, so liq_score zeroed out {raw_enchant_value:,.0f} of real enchant book value entirely")
    print()
...
def main():
    ...
    print(f"loaded {len(bazaar_prices)} bazaar products from DB")
    if not bazaar_prices:
        print("!! bazaar_prices table is empty")
    ...
    if not rows:
        print("no matching rows found in ended_auctions")
        conn.close()
        return
